...
|---|---|---|---|
| `separability_bench_*.{json,txt}` | `eval/separability_bench.py` | Can acoustic features separate RAVDESS arousal vs valence? | Arousal yes (`energy_std` AUC 0.89); valence ~chance (0.66). The one robust finding. |
...
| `ab_tag_response_*.{json,txt}` | `eval/ab_tag_response.py` | Do inline tags change LLM responses, zero-shot, across 6 models? | 100% "adapted" on the soft judge. Optimistic; the hard evals below deflate it. |
| `ab_hard_*.{json,txt}` | `eval/ab_tag_response_hard.py` | Same, strict judge + placebo + congruent/incongruent. | Effect falls to 36% substantive; placebo fires 28% (judge over-fires). |
...
| `ab_disentangle_*.{json,txt}` | `eval/ab_disentangle.py` | Is the value the arousal dimension or the emotion word? | Arousal tag competitive (MVP ok); emotion-word semantics carry more, which an energy tagger can't produce. |
